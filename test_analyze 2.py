...
def overstated_permissions(file_path, remaining_permissions, files):
    file = open(file_path)
	...
	if(i.find("startActivity") > -1):
	   ...
	   if (tmp not in file_name_list and "$" not in tmp):
	      print(tmp) 
...
for file_path in files_to_investigate:
    ...
    remaining_permissions = overstated_permissions(
        file_path, remaining_permissions, files_to_investigate)
